...
class Inference():
    ...
    def load_target_data(self):
        ...
        if os.path.isdir(self.target_path):
            ...
            # Check if file is image
                ...
                image_files, fps = extract_frames(self.target_path,
                                                  max_frames=100)  # Add a max number of frames to extract if needed
            else:
                print('Please specify correct target path. Extension should be .png, .jpg or .mp4')
                exit()
        else:
            print(
                'Please specify correct target path: directory with images or image file (.png, .jpg) or video (.mp4)')
            exit()
    ...
    def run_reenactment(self):
        ...
        for i, target_image in enumerate(tqdm(target_images)):
            ...
            print("reenacted_image: {}".format(reenacted_image.shape))

            if self.save_images:
                target_index = '{:06d}.png'.format(i)

                tensor2image(reenacted_image.cpu(), adaptive=True).save(os.path.join(self.output_path, target_index),
                                                                        "JPEG", quality=75, subsampling=0,
                                                                        progressive=True)
...
def main():
    ...
    ######### Generator #########
    parser.add_argument('--model_path', type=str, default='./pretrained_models/hypernetwork.pt',
                        help="set pre-trained e4e model path")

    # Default is False. Use argument for True
    parser.add_argument('--save_grids', dest='save_grids', action='store_true',
                        help="save results on a grid (source, target, reenacted)")
    ...
    parser.add_argument('--save_video', dest='save_video', action='store_true',
                        help="save results on video (source, target, reenacted)")
